...
RAW_Folder = "E:\\tempdata\\mbaumann\\Composite_Building\\RAW\\"
AA_output = "E:\\tempdata\\mbaumann\\Composite_Building\\" + PathRow + "\\"
workDir = "E:\\tempdata\\mbaumann\\Composite_Building\\RAW\\"
...
for folder in folderList[:]:
	if folder.find(".tar.gz") >=0 or folder.find("bin_image") >= 0 or folder.find("Thumbs.db") >= 0:				# Bin-image also remove from lsit so that we don not have to delete the temporary masks all the time
...
folderList = os.listdir(RAW_Folder)
for folder in folderList[:]:
	if folder.find("Thumbs.db") >=0 or folder.find("bin_image") >= 0:				# Bin-image also remove from lsit so that we don not have to delete the temporary masks all the time
		folderList.remove(folder)
...
for folder in folderList:
	...
	file_out = workDir + folder + "_bin_image.tif"
	if not os.path.exists(file_out):
		workDir_tmp = RAW_Folder + folder + "\\"
		list = os.listdir(workDir_tmp)
		for item in list[:]:
			if item.find("txt") >= 0 or item.find("GTF") >= 0 or item.find("B6") >= 0 or item.find("B8") >= 0 or item.find("_bin") >= 0 or item.find("_VER") >= 0 or item.find("Thumbs.db") >= 0:
				list.remove(item)
		for item in list:
			print(item)
