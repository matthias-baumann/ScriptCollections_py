...
source_folder = "E:\\tempdata\\mbaumann\\Composite_Building\\" + PathRow + "\\"
rule_folder = "E:\\tempdata\\mbaumann\\Composite_Building\\" + PathRow + "\\Compositing-rules\\"
output_folder = "E:\\tempdata\\mbaumann\\Composite_Building\\" + PathRow + "\\Composites\\"
	...
	if not os.path.exists(final_composite):					# DON'T FORGET TO ADD THE 'NOT'
		...
		for band in bands:
			...
			for y in range(rows):
				...
				exec(command)
				command = "np.putmask(dataOut, cloud == 7, 0)"		# eliminate Clouds --> class 7
				exec(command)
				command = "np.putmask(dataOut, cloud == 6, 0)"		# eliminate Sensor Noise --> class 6
				exec(command)
				command = "np.putmask(dataOut, cloud == 4, 0)"		# eliminate Cloud Shadow --> class 4
				exec(command)
				command = "np.putmask(dataOut, cloud == 3, 0)"		# eliminate Snow --> class 3
				exec(command)
				command = "np.putmask(dataOut, cloud == 2, 0)"		# eliminate Haze --> class 2
				exec(command)
				NrI = NrI + 1
				while NrI < NrImages:
					...
					command = "cloud = " + cGDAL_List[NrI]
					exec(command)
					command = "np.putmask(image, cloud == 7, 0)"		# eliminate Clouds --> class 7
					exec(command)
					command = "np.putmask(image, cloud == 6, 0)"		# eliminate Sensor Noise --> class 6
					exec(command)
					command = "np.putmask(image, cloud == 4, 0)"		# eliminate Cloud Shadow --> class 4
					exec(command)
					command = "np.putmask(image, cloud == 3, 0)"		# eliminate Snow --> class 3
					exec(command)
					command = "np.putmask(image, cloud == 2, 0)"		# eliminate Haze --> class 2
					exec(command)
