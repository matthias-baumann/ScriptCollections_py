...
# (1-A) Load image into GDAL, define cols and rows, create output file
	print("Find confident 'forest' training data using dark object approach in moving window.")
...
# (1-D) Start the operations
			# Mask out water from image and save water mask later --> NDVI-Threshold is here 0.2
...
# (4) Get less pure pixels for non-forest
	...
	output = output_folder + file + "_training-data"
...
# (5-E) Delete all files from the input steps - except 'training data', 'forestness Index' and 'water-mask'
	F_samples = output_folder + file + "_ForestSamples"
	F_samples_hdr = output_folder + file + "_ForestSamples.hdr"
	NF_samples = output_folder + file + "_ForestNonForestSamples"
	NF_samples_hdr = output_folder + file + "_ForestNonForestSamples.hdr"
	os.remove(F_samples)
	os.remove(F_samples_hdr)
	os.remove(NF_samples)
	os.remove(NF_samples_hdr)
