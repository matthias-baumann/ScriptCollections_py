...
coord_file = root_Folder + "\\" + center_PathRow + "_Corner-Coordinates.txt"
...
# (2-A) Create List for the scenes to be fitted, and remove Coordinate-File
cropScenes = os.listdir(root_Folder)
for scene in cropScenes[:]:
	if scene.find("Corner-Coordinates") >= 0:
		cropScenes.remove(scene)
	
# (2-B) First with the images
for scene in cropScenes[:]:
	workDir = root_Folder + scene + "\\"
	fileList = os.listdir(workDir)
	for file in fileList[:]:
		if file.find(".hdr") < 0 and file.find(".txt") < 0 and file.find("lndsr.") < 0 and file.find("Thumbs") < 0 and file.find("cropped") < 0 and file.find("Fmask") < 0:
			file01 = workDir + file
			...
			if os.path.exists(aux_File):
				os.remove(aux_File)
# (2-C) Now define overlap across bands --> important for SLC-off
