...
#'
#' @title aa_card
#' @description Calculates Agreement and Area of disproportioned stratified sampling
#' @param data confusion matrix or cbind(reference,map)
#' @param w stratum weights
#' @param area total area (optional)
...
aa_card <- function(data, w=NULL, area=NULL, confusion_matrix=T) {
  ...
  temp <-  p*(w_j_m-p)/n_j_m
#  temp[is.na(temp)] <- 0
  p_i_se <- sqrt(apply(temp ,2, FUN=function(x) sum(x, na.rm=T)))
#  p_i_se[is.na(p_i_se)] <- 0
...
#  p_i_se0  <- (sqrt(apply(p_i_se0, 2, sum)))
  ...
  df <- data.frame(class=class, cbind(ua, pa, p_i, p_i_se, ua_se, pa_se))
  df$w <- w_j
