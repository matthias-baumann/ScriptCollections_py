...
AnikaDir = "F:\\DataF\\mbaumann\\Data Processing\\06_Kelly_Sampling-design-chapter03\\Anika_Sample\\"
...
MODISfile = workDir + "MODIS_Evergreen_YesNo.tif"
CityDisFile = workDir + "Distance-to-Cities.tif"
MosStPeDisFile = workDir + "Distance-to-Moscow-StPetersburg.tif"
roadDisFile = workDir + "Distance-to-road_meters.tif"
districtFile = workDir + "District-boundaries.shp"
regionFile = workDir + "Region-Boundaries.shp"
parkFile = AnikaDir + "Parks_Merge.shp"
nightTime1993 = workDir + "F1F101993.cloud1.light1.round.byte.tif"
nightTime2003 = workDir + "F152003.cloud1.light1.round.byte.tif"
forestEdge1985 = AnikaDir + "1985_distance-to-forest-edge.tif"
forestEdge1990 = AnikaDir + "1990_distance-to-forest-edge.tif"
forestEdge1995 = AnikaDir + "1995_distance-to-forest-edge.tif"
forestEdge2000 = AnikaDir + "2000_distance-to-forest-edge.tif"
forestEdge2005 = AnikaDir + "2005_distance-to-forest-edge.tif"
forestEdge2010 = AnikaDir + "2010_distance-to-forest-edge.tif"
F_NF_1985 = AnikaDir + "1985_nfn_mos.img"
F_NF_1990 = AnikaDir + "1990_nfn_ext.img"
F_NF_1995 = AnikaDir + "1995_nfn_ext.img"
F_NF_2000 = AnikaDir + "1998_2000_nfn_ext.img"
F_NF_2005 = AnikaDir + "2006_nfn_ext.img"
F_NF_2010 = AnikaDir + "2009_2010_fnf_ext.img"

# BUILD LISTS FOR POINT-FILES
pointList = []
fileList = os.listdir(AnikaDir)
for file in fileList[:]:
	if file.find("Sample_") >= 0 and file.find(".shp") >= 0:
		pointList.append(file)
for file in pointList[:]:
	if file.find(".xml") >= 0 or file.find("Parks") >= 0 or file.find("withInfo") >= 0:
		pointList.remove(file)

for file in pointList:
	pointFileInput = AnikaDir + file
	...
	if not os.path.exists(pointFileOutput):
		...
		if not os.path.exists(outputTMP18):
			...
			# Rename fields, so that they fit Kelly's needs
			gp.AddField_management(outputTMP18, "REGION", "TEXT")
			rows = gp.UpdateCursor(outputTMP18)
			row = rows.next()
			while row:
				row.REGION = row.LINK
				rows.UpdateRow(row)
			...
			del row
			del rows
			gp.DeleteField_management(outputTMP18,"LINK")
			gp.AddField_management(outputTMP18, "DISTRICT", "TEXT")
			rows = gp.UpdateCursor(outputTMP18)
			row = rows.next()
			while row:
				row.DISTRICT = row.RNAME_ENG
				rows.UpdateRow(row)
				row = rows.next()
			del row
			del rows
			gp.DeleteField_management(outputTMP18,"RNAME_ENG")
			gp.AddField_management(outputTMP18, "DISTR_ID", "TEXT")
			rows = gp.UpdateCursor(outputTMP18)
			row = rows.next()
			while row:
				row.DISTR_ID = row.Unique_ID
				rows.UpdateRow(row)
				row = rows.next()
			del row
			del rows
			gp.DeleteField_management(outputTMP18,"Unique_ID")
			# Delete unneeded fields
			gp.DeleteField_management(outputTMP18,"EN_km2")
			gp.DeleteField_management(outputTMP18,"DN_km2")
			gp.DeleteField_management(outputTMP18,"EB_km2")
			gp.DeleteField_management(outputTMP18,"DB_km2")
			gp.DeleteField_management(outputTMP18,"Mixed_km2")
			gp.DeleteField_management(outputTMP18,"Sum_km2")
			gp.DeleteField_management(outputTMP18,"Join_Count")
		else:
		...
		if not os.path.exists(outputTMP19):
			...
			# Delete unneeded fields
			gp.DeleteField_management(outputTMP19,"Quality")
			gp.DeleteField_management(outputTMP19,"Join_Count")
			gp.DeleteField_management(outputTMP19,"AREA_HA")
			gp.DeleteField_management(outputTMP19,"NAME_E")
			gp.DeleteField_management(outputTMP19,"CODE")
			gp.DeleteField_management(outputTMP19,"TYPE")
			gp.DeleteField_management(outputTMP19,"Zone_oil")
			gp.DeleteField_management(outputTMP19,"Unique_Id")
			gp.DeleteField_management(outputTMP19,"Park")
			gp.DeleteField_management(outputTMP19,"BIOSPHERE")
			gp.DeleteField_management(outputTMP19,"Area_Alb")
		else:
		...
		deleteList = os.listdir(AnikaDir)
		for file in deleteList:
			if file.find("TMP") >= 0:
				delete = AnikaDir + file
				os.remove(delete)
		print "--------------------------------------------------------"
		print " "
	else:
