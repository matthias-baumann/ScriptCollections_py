...
loggingFile = "F:/DataF/kwendland/gistrials/GP_change/Regions_summary/Logging_sites.shp"
regionFile = "F:/DataF/kwendland/gistrials/GP_change/Regions_summary/Regions-intersect-Oblast_subset_with-Oblast-and-link-name.shp"
outputFile = "F:/DataF/kwendland/gistrials/GP_change/Regions_summary/region_logging_with_Oblast_Name.csv"

# CREATE OUTPUT FILE TO WRITE TO
fout = open(outputFile, "w")
fout.write("Region_ID,RNAME_ENG,Oblast_Name,Linked_Name,LOGSITES_NUM,LOGSITES_AREAM2,LOGSITES_AREAHA\n")
...
while row:
	if row.ID not in idList:
		...
		oblastList.append(row.ONAME_ENG)
		linkList.append(row.LINKED_OBL)
	row = rows.next()
...
for ID in idList:
	...
	fout.write(str(ID) + "," + regionList[i] + "," + oblastList[i] + "," + linkList[i] + ",") 
