...
# Make a spatial selection in the eco_lyr based on the UTM-Zone, CS is the same in both
    ...
    if eco_lyr.GetFeatureCount() == 0:
        ...
        # Process the job by extracting the correpsond values from GEE
                ...
                jobList.append(jobCollection)
                packageCount = 1
                job = []
    # take the last remaining jobs and append them
        jobCollection = ee.FeatureCollection(ee.List(job))
        jobList.append(jobCollection)
    # Now apply the joblist to extract the EE-numbers
        # Convert the UTM-Zone polygon into an EE-Feature, so that
        ...
        # Get the information about the pixels from the layers
        for job in tqdm(jobList):
            ...
            while processFlag == 0:
                try:
                    ...
                    # Switch the processing flag to 1 if succesfully processed
                    processFlag = 1
                except:
                    time.sleep(1)
                    print("--> GEE timed out, idle for 2min, ", time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime()))
                    time.sleep(120)
            ...
            for f25, f40, loss in zip(forest_25, forest_40, lossYR):
                ...
                vals = [f25_num['ORIG_FID'], f25_num['ECO_ID'], f25_num['ECO_NAME'], f25_num['Navin_Name'], f25_num['BIOME'], f25_num['propAREA'],
                        format(f25_num['sum'] * 900 / 1000000, '.5f'), format(f40_num['sum'] * 900 / 1000000, '.5f')]
