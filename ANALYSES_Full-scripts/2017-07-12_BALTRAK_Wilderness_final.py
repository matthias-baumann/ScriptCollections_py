...
ref = gdal.Open(baseFolder + "03_CroplandData/1990_PercGrassland_300m.tif", GA_ReadOnly)
...
# Prepare the grassland layers
    grass90 = Reproject(gdal.Open(baseFolder + "03_CroplandData/1990_PercGrassland_300m.tif", GA_ReadOnly), shpRas)
    grass15 = Reproject(gdal.Open(baseFolder + "03_CroplandData/2015_PercGrassland_300m.tif", GA_ReadOnly), shpRas)
...
# Find max and min values for the normalization
    cropMinMax = FindMinMax(grass90, grass15)
...
# Normalize the layers based on the minimum and maximum values
    #grass90norm = MaskRaster(InvertValues(Normalize(grass90, cropMinMax[0], cropMinMax[1])), shpRas, 1)
    #grass15norm = MaskRaster(InvertValues(Normalize(grass15, cropMinMax[0], cropMinMax[1])), shpRas, 1)
    grass90norm = InvertValues(Normalize(grass90, cropMinMax[0], cropMinMax[1]))
    grass15norm = InvertValues(Normalize(grass15, cropMinMax[0], cropMinMax[1]))
...
# Invert rasters for settlemtns and livestock stations to represent them as measures of human influence
    ...
    settDis90norm = InvertValues(settDis90norm)
    settDis15norm = InvertValues(settDis15norm)
    ZimLetDist90norm = InvertValues(ZimLetDist90norm)
    ZimLetDist15norm = InvertValues(ZimLetDist15norm)
# Calculate Wilderness, make binary information
    #wild90 = MaskRaster(CalcWindernessSum([grass90norm, settDis90norm, ZimLetDist90norm]), shpRas, 1)
    #wild15 = MaskRaster(CalcWindernessSum([grass15norm, settDis15norm, ZimLetDist15norm]), shpRas, 1)
    wild90 = CalcWindernessSum([grass90norm, settDis90norm, ZimLetDist90norm])
    wild15 = CalcWindernessSum([grass15norm, settDis15norm, ZimLetDist15norm])
# Calculate differences
    #grassDiff = MaskRaster(CalcDiff(grass90norm, grass15norm), shpRas, 1)
    ...
    grassDiff = CalcDiff(grass90norm, grass15norm)
    settDiff = CalcDiff(settDis90norm, settDis15norm)
    ZimLetDiff = CalcDiff(ZimLetDist90norm, ZimLetDist15norm)
    wildDiff =CalcDiff(wild90, wild15)
# Write Outputs
    baumiTools.baumiRT.CopyMEMtoDisk(grass90norm, outputFolder + "Thresh_" + str(th) + "_PercGrass_1990_NonMasked.tif")
    baumiTools.baumiRT.CopyMEMtoDisk(grass15norm, outputFolder + "Thresh_" + str(th) + "_PercGrass_2015_NonMasked.tif")
    baumiTools.baumiRT.CopyMEMtoDisk(settDis90norm, outputFolder + "Thresh_" + str(th) + "_DistToSett_1990_NonMasked.tif")
    baumiTools.baumiRT.CopyMEMtoDisk(settDis15norm, outputFolder + "Thresh_" + str(th) + "_DistToSett_2015_NonMasked.tif")
    baumiTools.baumiRT.CopyMEMtoDisk(ZimLetDist90norm, outputFolder + "Thresh_" + str(th) + "_DistToLivestock_1990_NonMasked.tif")
    baumiTools.baumiRT.CopyMEMtoDisk(ZimLetDist15norm, outputFolder + "Thresh_" + str(th) + "_DistToLivestock_2015_NonMasked.tif")
    baumiTools.baumiRT.CopyMEMtoDisk(wild90, outputFolder + "Thresh_" + str(th) + "_WildernessIndex_1990_NonMasked.tif")
    baumiTools.baumiRT.CopyMEMtoDisk(wild15, outputFolder + "Thresh_" + str(th) + "_WildernessIndex_2015_NonMasked.tif")
    baumiTools.baumiRT.CopyMEMtoDisk(grassDiff, outputFolder + "Thresh_" + str(th) + "_PercGrass_Change_NonMasked.tif")
    baumiTools.baumiRT.CopyMEMtoDisk(settDiff, outputFolder + "Thresh_" + str(th) + "_DistToSett_Change_NonMasked.tif")
    baumiTools.baumiRT.CopyMEMtoDisk(ZimLetDiff, outputFolder + "Thresh_" + str(th) + "_DistToLivestock_Change_NonMasked.tif")
    baumiTools.baumiRT.CopyMEMtoDisk(wildDiff, outputFolder + "Thresh_" + str(th) + "_WildernessIndex_Change_NonMasked.tif")
