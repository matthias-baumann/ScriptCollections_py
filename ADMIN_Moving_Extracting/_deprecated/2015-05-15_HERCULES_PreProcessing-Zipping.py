...
def Ctrans_MEM(inband_MEM, file_t_srs):
    ...
    return out_MEM
# ####################################### GLOBAL PATHS ######################################################## #
in_root = "G:/LEDAPS/outgoing/"
out_root = "G:/LEDAPS/Test/01_PreProcessing/02_ReProjected_Scenes/"
LedapsFmaskOut = "G:/HERCULES/01_LEDAPS_processed/"

ref_raster = "G:/HERCULES/01_PreProcessing/03_MetaData_Container_Output/PBC_container_HERCULES_multiYear_1984-1993_Cyprus.bsq"
# ####################################### START PROCESSING #################################################### #
...
# (2) GO THROUGH EACH ITEM IN THE LIST, DO THE PRE-PROCESSING
item = processList[0]
...
txt = [item + file for file in os.listdir(item) if file.find("GCP")>0 or file.find("MTL")>0]
# (2-1) COPY THE BANDS INTO MEMORY
...
#acca_landwater = OpenRasterToMemory('HDF4_EOS:EOS_GRID:' + lndsr[0] + ":Grid:land_water_QA")
#acca_adj_cloud = OpenRasterToMemory('HDF4_EOS:EOS_GRID:' + lndsr[0] + ":Grid:adjacent_cloud_QA")
...
# (2-5) ATMOSPHERIC OPTICAL THICKNESS
print("AOT, " + item)
aot_trans = Ctrans_MEM(aot, ref_raster)

# (2-6) THERMAL INFRARED
print("TIR, " + item)
...
# (2-9) WRITE NEW, PRE-PROCESSED FILES TO DISK
