...
stream = drvMemV.CopyDataSource(ogr.Open("D:/Matthias/Projects-and-Publications/Publications/Publications-in-preparation/Butsic-etal_Marihuana-California/_shpFiles_prepared/11_Streams.shp"),'')
streamfield = "Id"
...
totalFeat = parcelLYR.GetFeatureCount()
...
# Get APN identifier, check if in skip-feature
    ...
    if not ID in skipIDs:
        print("Processing APN " + str(ID) + ", Parcel " + str(counter) + " of " + str(totalFeat))
        vals = []
        vals.append(ID)
# Get the Geometry of the feature, transform to stream-coordinate systems
        ...
        streamLYR.SetSpatialFilter(geomParcel)
        streamsInParcel = streamLYR.GetFeatureCount()
...
# Append vals to outlist, reset reading of THP-Layer
        ...
        streamLYR.ResetReading()
        counter = counter + 1
        streamLYR.SetSpatialFilter(None)
        parcelFeat = parcelLYR.GetNextFeature()
    else:
        counter = counter + 1
        streamLYR.SetSpatialFilter(None)
