...
# ####################################### FOLDER PATHS AND BASIC VARIABLES FOR PROCESSING ##################### #
    ...
    out_csv = rootFolder + "_DataSummaries/PRIO-summaries_conflict_ALL_20191217.csv"
    ...
    # (1) calculate the summaries
            ...
            # (b) Nr. fatalities
                ...
                while conf_feat:
                    ...
                    conf_feat = conf_lyr.GetNextFeature()
            # (c) Average # fatalities per events
                if nr_events > 0:
            ...
            vals = [UID, nr_events, nr_fatalities, format(fat_per_event, '.3f')]
...
# (4) Merge the different packages back together into one dataset, instantiate colnames first
    print("Merge Outputs")
    outDS = [["PolygonID", "nr_events", "nr_fatalities", "nr_fatalities_per_event"]]
