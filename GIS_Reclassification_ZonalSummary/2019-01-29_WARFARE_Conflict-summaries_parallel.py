...
# ####################################### FOLDER PATHS AND BASIC VARIABLES FOR PROCESSING ##################### #
    ...
    point_shp = rootFolder + "_Variables/CONFLICT_DATA/PRIO/Disaggregated_Data/UCDP Georeferenced Event Dataset (GED) Global version 18.1 (2018)/ged181_type3.shp"
    out_csv = rootFolder + "_DataSummaries/PRIO-summaries_type3_bestEstimate_20191217.csv"
    yrs = range(2000, 2019)
    nPackages = 1000
    nr_cores = 50
# ####################################### PROCESSING ########################################################## #
